...
const CountryMovies = () => {
  const [, params] = useRoute('/quoc-gia/:country');
  const { getMoviesByCountry, isLoading } = useMovies();
  const [currentPage, setCurrentPage] = useState(1);
  const moviesPerPage = 20;
  
  const countrySlug = params?.country || '';
  const countryName = getUrlFromSlug(countrySlug);
  
  // Get movies by country
  const countryMovies = getMoviesByCountry(countrySlug);
  
  // Calculate pagination
  const indexOfLastMovie = currentPage * moviesPerPage;
  const indexOfFirstMovie = indexOfLastMovie - moviesPerPage;
  const currentMovies = countryMovies.slice(indexOfFirstMovie, indexOfLastMovie);
  const totalPages = Math.ceil(countryMovies.length / moviesPerPage);
  
  useEffect(() => {
    document.title = `Phim ${countryName.charAt(0).toUpperCase() + countryName.slice(1)} - FilmHay`;
    window.scrollTo(0, 0);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-white mb-8">
        Phim {countryName.charAt(0).toUpperCase() + countryName.slice(1)}
      </h1>
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-800/50 p-8 rounded-lg text-center">
          ...
          <p className="text-gray-400">
            Rất tiếc, chúng tôi không tìm thấy phim quốc gia {countryName} nào.
          </p>
        </div>
      )}
    </div>
