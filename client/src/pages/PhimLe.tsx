...
const PhimLe = () => {
  ...
  // Filter movie type movies
  const singleMovies = movies.filter(movie => movie.type === 'movie');
  
  // Calculate pagination
  const indexOfLastMovie = currentPage * moviesPerPage;
  const indexOfFirstMovie = indexOfLastMovie - moviesPerPage;
  const currentMovies = singleMovies.slice(indexOfFirstMovie, indexOfLastMovie);
  const totalPages = Math.ceil(singleMovies.length / moviesPerPage);
  
  useEffect(() => {
    document.title = 'Phim Lẻ - FilmHay';
    window.scrollTo(0, 0);
  }, [currentPage]);
