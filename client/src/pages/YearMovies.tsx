...
const YearMovies = () => {
  const [, params] = useRoute('/nam/:year');
  const { getMoviesByYear, isLoading } = useMovies();
  ...
  const currentMovies = yearMovies.slice(indexOfFirstMovie, indexOfLastMovie);
  const totalPages = Math.ceil(yearMovies.length / moviesPerPage);
  
  useEffect(() => {
    document.title = `Phim năm ${year} - FilmHay`;
    window.scrollTo(0, 0);
  }, [year, currentPage]);
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-800/50 p-8 rounded-lg text-center">
          ...
          <p className="text-gray-400">
            Rất tiếc, chúng tôi không tìm thấy phim nào sản xuất năm {year}.
          </p>
        </div>
      )}
    </div>
