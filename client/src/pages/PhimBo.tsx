...
const PhimBo = () => {
  ...
  // Filter series type movies
  const seriesMovies = movies.filter(movie => movie.type === 'series');
  
  // Calculate pagination
  const indexOfLastMovie = currentPage * moviesPerPage;
  const indexOfFirstMovie = indexOfLastMovie - moviesPerPage;
  const currentMovies = seriesMovies.slice(indexOfFirstMovie, indexOfLastMovie);
  const totalPages = Math.ceil(seriesMovies.length / moviesPerPage);
  
  useEffect(() => {
    document.title = 'Phim Bộ - FilmHay';
    window.scrollTo(0, 0);
  }, [currentPage]);
  
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-white mb-8">Phim Bộ</h1>
